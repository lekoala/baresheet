...
/**
 * Compare seekable writeFile() and non-seekable output() XLSX paths.
 *
 * Usage:
 *   php bin/bench-xlsx-stream.php [rows]
 */
...
function streamRows(int $rows): Generator
{
    for ($i = 1; $i <= $rows; $i++) {
        yield [
...
function streamMedian(array $values): float
{
    sort($values);
...
if (($argv[1] ?? '') === '--worker') {
    ...
    if ($mode === 'seekable') {
        $writer->writeFile(streamRows($rows), $output);
    } elseif ($mode === 'non-seekable') {
        $writer->output(streamRows($rows), 'streamed.xlsx');
    } else {
...
for ($rep = 0; $rep < STREAM_REPS; $rep++) {
    $order = array_keys($results);
    shuffle($order);

    foreach ($order as $mode) {
        $base = tempnam(sys_get_temp_dir(), 'bench_xlsx_stream_');
        if ($base === false) {
        ...
        if ($open !== true) {
            throw new RuntimeException("Invalid {$mode} archive, ZipArchive code: {$open}");
        }
        $zip->close();
        unlink($output);
    }
...
echo "# XLSX seekable vs non-seekable ({$rows} rows × 10 columns)\n\n";
...
foreach ($results as $mode => $values) {
    printf(
        "| %s | %.4f | %.2f | %.2f |\n",
        $mode,
        streamMedian(array_column($values, 'seconds')),
        streamMedian(array_column($values, 'peakMemory')) / 1024 / 1024,
